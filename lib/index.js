...
function apply(ctx, config) {
    ...
    // 添加到合并队列
    const addToMerge = (sessionId, videoData, session) => {
            ...
            try {
                ...
                if (response.code === 200 && response.data) {
                    ...
                    if (!addedToMerge) {
                        // 如果没有启用合并转发，直接发送
                        await session.send([
                            (0, koishi_1.h)('image', { src: data.cover_url }),
                            (0, koishi_1.h)('text', `标题：${data.title}`),
                            (0, koishi_1.h)('text', `作者：${data.author.name}`)
                        ]);
                    }
                    else {
                        if (config.debug) {
